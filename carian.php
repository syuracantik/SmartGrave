...
:root{
  ...
  --shadow:0 10px 25px -5px rgba(15, 23, 42, 0.06), 0 8px 10px -6px rgba(15, 23, 42, 0.06);
  --shadow-md:0 20px 40px -15px rgba(15, 23, 42, 0.1);
  --radius:20px;
  --radius-sm:12px;
  --header-bg:linear-gradient(135deg,#064e3b 0%,#022c22 100%);
  --font-display:'Plus Jakarta Sans', sans-serif;
  --font-sans:'Inter', sans-serif;
  --font-mono:'DM Mono', monospace;
}
*{margin:0;padding:0;box-sizing:border-box}
html,body{height:100%;overflow:hidden;font-family:var(--font-sans);color:var(--text);background-color:var(--bg);background-image:url("https://www.transparenttextures.com/patterns/arabesque.png");}
...
.logo{display:flex;align-items:center;gap:10px;text-decoration:none;transition:opacity 0.2s}
...
.logo-text{font-family:var(--font-display);font-size:18px;font-weight:800;color:#fff;letter-spacing:-.4px}
.logo-text em{color:#34d399;font-style:normal}
.h-sep{width:1px;height:22px;background:rgba(255,255,255,.18);margin:0 2px}
.h-badge{
...
.h-space{flex:1}
.h-dot{width:7px;height:7px;border-radius:50%;background:#4ade80;animation:blink 2s infinite;flex-shrink:0}
...
.h-counts{display:flex;gap:8px;margin-left:12px}
.h-count{
...
.h-count strong{display:block;font-size:15px;font-weight:700;color:#fff;font-family:var(--font-mono);line-height:1.2}
...
.sidebar{
  ...
  overflow:hidden;
  box-shadow:4px 0 24px rgba(15,23,42,.02);
}
...
.s-btn{
  ...
  width:46px;color:#fff;font-size:15px;
  cursor:pointer;flex-shrink:0;
  transition:all .2s ease;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 12px rgba(16,185,129,.2);
}
.s-btn:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(16,185,129,.3)}
.s-btn:active{transform:scale(.97)}

.pills{display:flex;gap:6px;margin-top:14px}
.pill{
  flex:1;text-align:center;
  padding:8px 0;border-radius:var(--radius-sm);font-size:12px;font-weight:700;
  cursor:pointer;border:1.5px solid var(--border);
  color:var(--muted);background:var(--surface);
  font-family:var(--font-display);
  transition:all .18s cubic-bezier(0.4, 0, 0.2, 1);
}
.pill:hover{border-color:#cbd5e1;background:var(--bg);color:var(--text)}
.pill.active-all{background:var(--green-xl);color:var(--green-l);border-color:rgba(16,185,129,.3)!important;box-shadow:0 2px 8px rgba(16,185,129,.08)}
.pill.active-a{background:var(--blue-l);color:var(--blue);border-color:rgba(37,99,235,.3)!important;box-shadow:0 2px 8px rgba(37,99,235,.08)}
.pill.active-b{background:var(--purple-l);color:var(--purple);border-color:rgba(124,58,237,.3)!important;box-shadow:0 2px 8px rgba(124,58,237,.08)}
...
.gps-dot{width:8px;height:8px;border-radius:50%;background:#10b981;animation:blink 1.5s infinite;flex-shrink:0}
#gpsStatus{flex:1;color:var(--muted);font-weight:600}
#userDistBadge{font-family:var(--font-mono);font-weight:700;color:#047857;font-size:11px}
.sim-btn{
...
.sim-btn:hover, .sim-btn.active{
  background:#10b981;color:#fff;box-shadow:0 2px 8px rgba(16,185,129,.2);
}

/* ── RESULTS ── */
.results{flex:1;overflow-y:auto;padding:16px 16px 80px;background:#fafafb}
.r-meta{
  display:flex;justify-content:space-between;align-items:center;
  padding:0 4px 12px;
}
.r-label{font-family:var(--font-display);font-size:11px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:var(--muted)}
.r-count{
  font-family:var(--font-mono);font-size:11px;font-weight:700;
  color:#047857;background:var(--green-xl);
...
.gcard.za::before{background:linear-gradient(180deg,#3b82f6,#1d4ed8)}
.gcard.zb::before{background:linear-gradient(180deg,#8b5cf6,#6d28d9)}
.gcard:hover{
...
.gcard.sel{
  ...
  box-shadow:0 0 0 4px rgba(16,185,129,.1),var(--shadow);
}
...
.ba{background:var(--blue-l);color:var(--blue);border:1px solid rgba(37,99,235,.15)}
.bb{background:var(--purple-l);color:var(--purple);border:1px solid rgba(124,58,237,.15)}
.gc-lot{font-family:var(--font-mono);font-size:11px;color:var(--muted);font-weight:700;background:#f1f5f9;padding:2px 8px;border-radius:4px}
.gc-name{font-family:var(--font-display);font-size:15px;font-weight:800;margin-bottom:6px;color:var(--text);line-height:1.35}
.gc-meta{font-size:11.5px;color:var(--muted);font-family:var(--font-sans);line-height:1.6}
...
.gc-nav-btn{
  ...
  border-radius:var(--radius-sm);
  font-family:var(--font-display);font-size:12.5px;font-weight:800;color:var(--green-l);
  cursor:pointer;
...
.empty i{font-size:42px;color:#cbd5e1}
...
.coord{
  ...
  border:1px solid var(--border);
  font-family:var(--font-mono);font-size:10px;color:var(--green-l);
...
.leg-ttl{
  font-family:var(--font-display);
  font-size:9.5px;font-weight:850;letter-spacing:1.5px;
...
.leg-sep{height:1px;background:rgba(15, 23, 42, 0.06);margin:10px 0}
...
.nav-hdr-info{flex:1}
.nav-hdr-sub{font-family:var(--font-display);font-size:9px;font-weight:850;letter-spacing:1.5px;text-transform:uppercase;color:rgba(255,255,255,0.55);margin-bottom:2px}
.nav-hdr-name{font-family:var(--font-display);font-size:14px;font-weight:800;color:#fff}
.nav-close{
...
.nav-prog{height:4px;background:rgba(16, 185, 129, 0.08);flex-shrink:0}
.nav-prog-bar{height:100%;background:linear-gradient(90deg, var(--green) 0%, var(--green-l) 100%);width:0%;transition:width .3s ease}

.nav-eta{
  display:flex;align-items:center;
  background:#022c22;padding:16px 20px;
  flex-shrink:0;
}
.nav-eta-time{font-size:32px;font-weight:800;color:#fff;font-family:var(--font-mono);line-height:1}
.nav-eta-unit{font-size:12px;color:rgba(255,255,255,0.6);margin-left:4px;margin-top:12px;font-weight:700}
.nav-eta-sep{width:1px;height:36px;background:rgba(255,255,255,.15);margin:0 20px}
.nav-eta-dist{font-size:18px;font-weight:800;color:#fff;font-family:var(--font-mono)}
.nav-eta-dlabel{font-family:var(--font-display);font-size:9px;color:rgba(255,255,255,0.5);font-weight:800;letter-spacing:0.8px;margin-top:2px}
...
.nav-eta-lot span{font-family:var(--font-display);font-size:9px;color:rgba(255,255,255,0.5);font-weight:800}
.nav-live-dist{
...
.nav-live-dot{width:8px;height:8px;border-radius:50%;background:#10b981;animation:blink 1.2s infinite;flex-shrink:0}
...
.step.s-first{border-color:rgba(16, 185, 129, 0.2)}
.step.s-dest{border-color:rgba(239, 68, 68, 0.2)}
.step.s-active{
  border-color:var(--green);
...
.step.s-first .step-icon{background:var(--green);color:#fff;border-color:var(--green)}
.step.s-dest .step-icon{background:var(--red);color:#fff;border-color:var(--red)}
.step.s-active .step-icon{background:var(--green-d);color:#fff;border-color:var(--green-d)}
.step-body{flex:1}
.step-act{font-family:var(--font-display);font-size:13px;font-weight:800;color:var(--text);margin-bottom:3px}
.step-det{font-size:11px;color:var(--muted);line-height:1.45}
.step-dist-badge{
  font-family:var(--font-mono);font-size:10px;font-weight:700;
  color:var(--green-l);background:var(--green-xl);
...
.ph-id{font-family:var(--font-mono);font-size:10px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;margin-bottom:2px}
.ph-name{font-family:var(--font-display);font-size:14px;font-weight:800;line-height:1.3}
.pb{padding:16px}
.pg{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px}
.pf{background:#f8fafc;border-radius:var(--radius-sm);padding:8px 10px;border:1px solid var(--border)}
.pf-l{font-size:8px;color:var(--muted);text-transform:uppercase;letter-spacing:.7px;margin-bottom:3px;font-family:var(--font-mono);font-weight:700}
.pf-v{font-size:11px;font-weight:700;font-family:var(--font-mono);color:var(--text)}
.pgps{background:var(--green-xl);border:1px solid rgba(16, 185, 129, 0.15);border-radius:var(--radius-sm);padding:8px 10px;margin-bottom:12px}
.pgps-l{font-size:8px;color:var(--green-l);text-transform:uppercase;letter-spacing:.7px;margin-bottom:3px;font-family:var(--font-mono);font-weight:700}
.pgps-v{font-size:10px;font-weight:700;font-family:var(--font-mono);color:var(--green-l)}
.p-imgs{display:flex;gap:8px;margin-bottom:12px}
.p-img-ph{
  flex:1;height:76px;border-radius:var(--radius-sm);border:1px solid var(--border);
  background-size:cover;background-position:center;
  transition:transform 0.2s;
}
...
.p-live-dist{
  display:flex;align-items:center;gap:6px;
  background:var(--green-xl);border:1px solid rgba(16, 185, 129, 0.15);
  border-radius:var(--radius-sm);padding:8px 12px;margin-bottom:12px;
  font-size:11px;font-weight:600;
}
.pld-dot{width:6px;height:6px;border-radius:50%;background:#10b981;animation:blink 1.5s infinite;flex-shrink:0}
.pld-val{font-weight:700;color:var(--green-l);font-family:var(--font-mono)}
.pld-lbl{color:var(--muted)}
.pnav{
  width:100%;padding:11px 14px;
  background:linear-gradient(135deg, var(--green) 0%, var(--green-l) 100%);border:none;border-radius:var(--radius-sm);
  font-family:var(--font-display);font-size:12.5px;font-weight:800;color:#fff;
  cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;
  transition:all 0.2s;box-shadow:0 4px 12px rgba(16, 185, 129, 0.25);
}
.pnav:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(16, 185, 129, 0.35)}
...
.pm{width:22px;height:22px;background:#10b981;border:3.5px solid #fff;border-radius:50%;animation:pulse 1.6s infinite;box-shadow:0 3px 8px rgba(15, 23, 42, 0.2)}
.user-loc-dot{width:18px;height:18px;background:#2563eb;border:3.5px solid #fff;border-radius:50%;box-shadow:0 0 0 4px rgba(37, 99, 235, 0.25), 0 3px 8px rgba(15, 23, 42, 0.2);animation:pulse 2s infinite}
.walk-dot{
  width:24px;height:24px;
  background:#064e3b;border:3px solid #fff;border-radius:50%;
    ...
    <div class="search-box">
      ...
      <div class="s-row">
        <div class="s-wrap">
          ...
          <input type="text" id="sInput" class="s-input" placeholder="Nama atau No. IC...">
        </div>
...
function getGraveImagesHTML(d, className = 'p-img-ph') {
  ...
  if (d.gambar_kiri && d.gambar_kiri.trim() !== '') {
    html += `<div class="${className}" style="background-image: url('${d.gambar_kiri}'); background-size: cover; background-position: center; border: 1px solid var(--border);" title="Batu Nisan"></div>`;
  }
  if (d.gambar_kanan && d.gambar_kanan.trim() !== '') {
    html += `<div class="${className}" style="background-image: url('${d.gambar_kanan}'); background-size: cover; background-position: center; border: 1px solid var(--border);" title="Kawasan Sekitar"></div>`;
  }
  if (d.gambar_penanda && d.gambar_penanda.trim() !== '') {
    html += `<div class="${className}" style="background-image: url('${d.gambar_penanda}'); background-size: cover; background-position: center; border: 1px solid var(--border);" title="Penanda Laluan"></div>`;
  }
  if (html !== '') {
...
function makePopup(d) {
  ...
  return `<div class="pw">
    ...
    <div class="pb">
      ${imgsHTML}
      <div class="pg">
        <div class="pf"><div class="pf-l">No. IC</div><div class="pf-v">${d.ic}</div></div>
...
function doSearch() {
  ...
  list.innerHTML=res.map(d=>{
    ...
    <div class="gcard z${d.zon.toLowerCase()}" id="gc-${d.id}" onclick="focusGrave('${d.id}')">
      ...
      <div class="gc-meta">IC: ${d.ic}<br>Lahir: ${d.lahir} · Wafat: ${d.mati}</div>
